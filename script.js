...
async function confirmSimulatedOrder() {
  ...
  // Forward order automatically to SQL Server ERP API Bridge
  try {
    fetch('/api/integration/web-order', {
      method: 'POST',
      ...
      body: JSON.stringify({
        customer: {
          ...
          address: `${address}, ${cityState} - ${pincode}`
        },
        items: items.map(i => ({
          sku: i.sku || `SKU-${i.id}`,
          productName: i.name,
        ...
        orderNotes: `Website Order ${result.order_number || ''}`
      })
    }).then(res => res.json()).then(erpRes => {
      if (erpRes && erpRes.erpOrder) {
        console.log('[ERP Integration] Order successfully created in ERP SalesNo:', erpRes.erpOrder.SalesNo);
      }
    }).catch(e => {
      console.warn('[ERP Integration] ERP API bridge notice:', e);
    });
  } catch (erpErr) {
    console.warn('[ERP Integration] Error forwarding to ERP:', erpErr);
  }
