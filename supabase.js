...
-- WHERE email = 'YOUR_ADMIN_EMAIL';
    ...
    async getShipments(){const c=this.getClient();if(!c)return[];const{data,error}=await c.from('shipments').select('*, orders(order_number,customer_name,customer_email,total_amount,payment_status,shipping_address)').order('created_at',{ascending:false});if(error){console.error(error);return[]}return data||[];},
    async getShipment(id){const c=this.getClient();if(!c)return null;const{data,error}=await c.from('shipments').select('*, orders(*)').eq('id',id).maybeSingle();if(error){console.error(error);return null}return data||null;},
    async updateShipment(row){const c=this.getClient();if(!c)return{success:false,error:'Supabase is not configured.'};const payload={...row,id:Number(row.id),order_id:Number(row.order_id)};for(const k of ['shipping_cost','cod_amount','weight_kg']) if(payload[k]!==undefined) payload[k]=Number(payload[k]||0);if(payload.status==='Shipped'&&!payload.shipped_at)payload.shipped_at=new Date().toISOString();if(payload.status==='Delivered'&&!payload.delivered_at)payload.delivered_at=new Date().toISOString();const{data,error}=await c.from('shipments').update(payload).eq('id',payload.id).select().single();if(error)return{success:false,error:error.message};return{success:true,shipment:data};},
    async setShipmentTracking(id, patch){return this.updateShipment({id,...patch});},
