    ...
    async function loadCategories() {
      ...
      tableBody.innerHTML = categories.map(cat => `
        <tr id="cat-row-${cat.id}">
          ...
          <td>
            <div class="action-buttons-group">
              <button type="button" class="btn-action edit" onclick="openEditCategory('${cat.id}', '${cat.name}', '${cat.slug}', '${(cat.description || '').replace(/'/g, "\\'")}')">Edit</button>
              <button type="button" class="btn-action delete" onclick="deleteCategory('${cat.id}', '${cat.name}')">Delete</button>
            </div>
          </td>
        </tr>
      `).join('');
