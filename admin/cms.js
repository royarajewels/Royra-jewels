(function () {
  ...
  function mediaCard(m) {
    return `<div class="cms-media-card" data-media-id="${esc(m.id)}">
      ...
      <div class="cms-row-actions">
        <button class="btn-action" type="button" data-copy="${esc(m.public_url || '')}">Copy URL</button>
        <button class="btn-action danger" type="button" data-delete-media="${esc(m.id)}">Delete</button>
      </div>
    </div>`;
  }
  ...
  window.initMediaLibrary = async function () {
    ...
    async function load() {
      ...
      grid.querySelectorAll('[data-copy]').forEach(btn => btn.onclick = async () => {
  ...
  window.initBannerManager = async function () {
    ...
    async function load() {
      ...
      tbody.innerHTML = banners.length ? banners.map(b => `<tr>
        ...
        <td><div class="action-buttons-group"><button class="btn-action edit" type="button" data-edit-banner="${b.id}">Edit</button><button class="btn-action danger" type="button" data-delete-banner="${b.id}">Delete</button></div></td>
  ...
  window.initCollectionManager = async function () {
    ...
    async function load() {
      ...
      tbody.innerHTML = collections.length ? collections.map(c => `<tr>
        ...
        <td><div class="action-buttons-group"><button class="btn-action edit" data-edit-collection="${c.id}">Edit</button><button class="btn-action danger" data-delete-collection="${c.id}">Delete</button></div></td>
      </tr>`).join('') : '<tr><td colspan="6" class="cms-empty">No collections yet.</td></tr>';
      tbody.querySelectorAll('[data-edit-collection]').forEach(b => b.onclick = () => fill(collections.find(x => String(x.id)===String(b.dataset.editCollection))));
